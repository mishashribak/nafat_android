...
public class AuthOTPService {
    private static final String TAG = VerifyMobileService.class.getSimpleName();

    public void start(OPTAuthRequest request, final ApiCallback callback) {

        final APIs apIs = APIBulldozer.createAPI(APIs.class);

        apIs.otpAuthentication(request)
